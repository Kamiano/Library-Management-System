...
class LibraryManagementSystem:
    def __init__(self, root):
        ...
        DataFrameRight = LabelFrame(frame, text="Book Details", bg="powder blue",
                                    fg="green", bd=20, relief=RIDGE, font=("Times New Roman", 12, "bold"))
        DataFrameRight.place(x=910, y=5, width=580, height=390)
...
# information frame
    ...
    def adda_data(self):  
        try:
            ...
            conn = pyodbc.connect(
                ...
                'SERVER=DESKTOP-4OVV0LM\SQLEXPRESS;'
                'DATABASE=Library management;'
                'Trusted_Connection=yes;'
            )
            my_cursor = conn.cursor()
            my_cursor.execute("INSERT INTO library VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (
                self.Member_var.get(),
                self.prn_no_var.get(),
                self.id_var.get(),
    ...
    def update(self):
        try:
            ...
            my_cursor = conn.cursor()
            my_cursor.execute("UPDATE library SET Member=?,id=?,first_name=?,last_name=?,address1=?,address2=?,postal_code=?,phone_no=?,book_id=?,book_title=?,author_name=?,date_borrowed=?,date_due=?,days_on_book=?,late_return_fine=?,date_over_due=?,actual_price=? where PRN_NO=?", (
                self.Member_var.get(),
                self.id_var.get(),
                self.firstname_var.get(),
                self.lastname_var.get(),
    ...
    def showData(self):
         self.txtBox.insert(END,"1:"+self.Member_var.get()+"\n")
         self.txtBox.insert(END,"2:"+self.prn_no_var.get()+"\n")
         self.txtBox.insert(END,"3:"+self.id_var.get()+"\n")
         self.txtBox.insert(END,"4:"+self.firstname_var.get()+"\n")
         self.txtBox.insert(END,"5:"+self.lastname_var.get()+"\n")
         self.txtBox.insert(END,"6:"+self.address1_var.get()+"\n")
         self.txtBox.insert(END,"7:"+self.address2_var.get()+"\n")
         self.txtBox.insert(END,"8:"+self.postcode_var.get()+"\n")
         self.txtBox.insert(END,"9:"+self.phone_var.get()+"\n")
         self.txtBox.insert(END,"10:"+self.book_id_var.get()+"\n")
         self.txtBox.insert(END,"11:"+self.book_title_var.get()+"\n")
         self.txtBox.insert(END,"12:"+self.author_var.get()+"\n")
         self.txtBox.insert(END,"13:"+self.dateborrowed_var.get()+"\n")
         self.txtBox.insert(END,"14:"+self.datedue_var.get()+"\n")
         self.txtBox.insert(END,"15:"+self.daysonbook_var.get()+"\n")
         self.txtBox.insert(END,"16:"+self.laterfine_var.get()+"\n")
         self.txtBox.insert(END,"17:"+self.dateoverdue_var.get()+"\n")
         self.txtBox.insert(END,"18:"+self.finalprice_var.get()+"\n")
    ...
    def delete(self):
     if self.prn_no_var.get() == "" or self.id_var.get() == "":
        ...
        try:
            ...
            my_cursor = conn.cursor()
            query = "DELETE FROM library WHERE PRN_NO = ?"
